...
struct NotificationCardView: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            // Left side: Team badge and player info
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 12) {
                    // Team badge
                    TeamBadgeView(teamName: notification.team, isOwned: notification.isOwned, size: 44, teamAbbreviation: notification.teamAbbreviation)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16)
                .stroke(
                    notification.isRead ? Color.clear : Color.gray.opacity(0.3), 
                    lineWidth: 1.5
                )
        )
        .shadow(
