#!/usr/bin/env python3
"""
FPL Mobile Monitor - Enhanced Service with Change Detection
==========================================================

Enhanced version with proper change detection, data persistence, and Supabase integration.
            ...
            for player_id, live_player in live_data.items():
                ...
                cursor.execute("""
                    ...
                    ON CONFLICT (player_id, fixture_id, gameweek) DO UPDATE SET
                        ...
                        updated_at = NOW()
                """, (
                    player_id, current_stats.get("fixture", 0), gameweek,
...
class FPLAPIClient:
    ...
    async def get_manager_data(self, manager_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.session.get(f"{config.fpl_base_url}/entry/{manager_id}/")
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_league_details(self, league_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.session.get(f"{config.fpl_base_url}/leagues-classic/{league_id}/standings/")
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
class FPLMonitoringService:
    ...
    def __init__(self):
        ...
        self.fpl_client = self.api  # Alias for API endpoints
...
async def get_monitoring_status():
    ...
    return {
        ...
        "supabase_connected": monitoring_service.db.supabase is not None
    }
...
async def search_managers(query: str = "", limit: int = 20):
    ...
    try:
        ...
        if query.isdigit():
            ...
            try:
                manager_data = await monitoring_service.fpl_client.get_manager_data(manager_id)
                if manager_data:
...
async def get_manager_leagues(manager_id: int):
    """Get leagues for a specific manager"""
    try:
        leagues_data = await monitoring_service.fpl_client.get_manager_leagues(manager_id)
        return leagues_data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_league_details(league_id: int):
    ...
    try:
        league_data = await monitoring_service.fpl_client.get_league_details(league_id)
        if not league_data:
